...
var options;
let gettingImagereplace = browser.storage.sync.get("imgreplace");
gettingImagereplace.then(function (details) { options = details["imgreplace"]; ConstructRows(); });

function LoadCheck(name) {
  let gettingCheck = browser.storage.sync.get(name)
  gettingCheck.then(function (details) { 
...
function SaveCheck(SaveName, SaveState) {
  ...
  browser.storage.sync.set(tempOption);
}
...
function SaveOptions(tempOptions = []) {
  ...
  browser.storage.sync.set({"imgreplace": tempOptions});
}
